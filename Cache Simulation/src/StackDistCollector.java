...
public class StackDistCollector
{
		public Integer access(int address)
		{
			...
			{
				SymbolRecord record = symbolTable.getRecord(address);
				int last_access = record.getLastAccessTime(); //to store the last access time
				int count = pastAccesses.countNoSmallerThan(last_access);   //accessing the distinct elements accessed after the the last access time of address
				pastAccesses.remove(last_access); // removed last access time
				pastAccesses.insert(time); // inserted the new time
				record.setLastAccessTime(time);    // set the new access time
				symbolTable.putRecord(address, record);
				time++;
				return count;
			}
			else
			{
				SymbolRecord new_record =new SymbolRecord(address, time); //new record for the new address
				symbolTable.putRecord(address, new_record);   // putting the new record in the table
				pastAccesses.insert(time);    // inserting the new time in sequence
